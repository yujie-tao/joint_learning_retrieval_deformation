...
parser.add_argument(
    "--config",
    default="config_full_chairs.json",
...
parser.add_argument(
    "--logdir",
    default="log_chair_keypoint_icpl2_srcenc_symm/",
...
if not os.path.exists(mesh_fol):
    os.mkdir(mesh_fol)
...
if __name__ == "__main__":
    ...
    if not ALL_MODELS:
        if COMPLEMENTME:
            ...
            src_data_fol = os.path.join(
                BASE_DIR, "data_complementme_final", OBJ_CAT, "h5_new"
            )
            ...
            src_data_fol = os.path.join(
                BASE_DIR, "data_aabb_constraints", OBJ_CAT, "h5"
            )

        if COMPLEMENTME:
            filename_pickle = os.path.join(
                "generated_datasplits_complementme",
                OBJ_CAT + "_" + str(NUM_SOURCES) + ".pickle",
            )
        else:
            filename_pickle = os.path.join(
        ...
        if COMPLEMENTME:
            filename = os.path.join(
                "generated_datasplits_complementme",
                OBJ_CAT + "_" + str(NUM_SOURCES) + "_" + DATA_SPLIT + ".h5",
            )
        else:
        ...
        filename_pickle = os.path.join(
            "generated_datasplits",
            "all_classes_chairtablecabinet_smaller.pickle",
        )
        sources, sources_cat, _, _ = get_all_selected_models_pickle(
            filename_pickle, all_models=True
        )
        filename = os.path.join(
            "generated_datasplits", "all_classes_test_smaller.h5"
        )
        ...
        if not ALL_MODELS:
            if USE_CONNECTIVITY:
                (
                    ...
                    os.path.join(src_data_fol, src_filename),
                    mesh=True,
                    constraint=True,
                )
            else:
            ...
            if USE_CONNECTIVITY:
                (
                    ...
                    os.path.join(
                        src_data_fol, sources_cat[i], "h5", src_filename
                    ),
                    mesh=True,
                    constraint=True,
                )
    ...
    for i, batch in enumerate(loader):
        ...
        target_latent_codes = target_latent_codes.unsqueeze(0).repeat(
            len(SOURCE_MODEL_INFO), 1, 1
        )
        source_labels = source_label_shape.unsqueeze(0).repeat(
            len(SOURCE_MODEL_INFO), 1
        )

        # Reshape to (K*batch_size, ...) to feed into the network
        # Source assignments have to be done accordingly
        target_latent_codes = target_latent_codes.view(
            -1, target_latent_codes.shape[-1]
        )
        source_labels = source_labels.view(-1)

        # Get all labels
        source_labels = get_all_source_labels(
            source_labels, len(SOURCE_MODEL_INFO)
        )

        # Also overwrite x for chamfer distance
        x_repeated = x.unsqueeze(0).repeat(len(SOURCE_MODEL_INFO), 1, 1, 1)
        x_repeated = x_repeated.view(
            -1, x_repeated.shape[-2], x_repeated.shape[-1]
        )
        ...
        src_mats, src_default_params, src_connectivity_mat = get_source_info(
            source_labels,
            SOURCE_MODEL_INFO,
            MAX_NUM_PARAMS,
        ...
        src_latent_codes = get_source_latent_codes_fixed(
            source_labels, SOURCE_LATENT_CODES, device
        )

        mat = [mat.to(device, dtype=torch.float) for mat in src_mats]
        def_param = [
            def_param.to(device, dtype=torch.float)
            for def_param in src_default_params
        ]
        ...
        if USE_CONNECTIVITY:
            conn_mat = [
                conn_mat.to(device, dtype=torch.float)
                for conn_mat in src_connectivity_mat
            ]
            conn_mat = torch.stack(conn_mat)

        concat_latent_code = torch.cat(
            (src_latent_codes, target_latent_codes), dim=1
        )

        # print(concat_latent_code.shape)

        # Param Decoder per part
        # Make the part latent codes of each source into a (K x
        # PART_LATENT_DIM) tensor
        all_params = []
        for j in range(concat_latent_code.shape[0]):
            ...
            curr_code_repeated = curr_code.view(1, curr_code.shape[0]).repeat(
                curr_num_parts, 1
            )

            part_latent_codes = SOURCE_PART_LATENT_CODES[source_labels[j]]

            full_latent_code = torch.cat(
                (curr_code_repeated, part_latent_codes), dim=1
            )
            ...
            if curr_num_parts < MAX_NUM_PARTS:
                dummy_params = torch.zeros(
                    (MAX_NUM_PARTS - curr_num_parts, embedding_size),
                    dtype=torch.float,
                    device=device,
                )
        ...
        if USE_CONNECTIVITY:
            output_pcs = get_shape(
                mat, params, def_param, ALPHA, connectivity_mat=conn_mat
            )
        else:
            output_pcs = get_shape(mat, params, def_param, ALPHA)

        cd_loss, _ = chamfer_distance(
            output_pcs, x_repeated, batch_reduction=None
        )

        output_pcs = output_pcs.view(
            len(SOURCE_MODEL_INFO),
            target_shapes.shape[0],
            target_shapes.shape[1],
            target_shapes.shape[2],
        )
        ...
        if JOINT_MODEL:
            ...
            if not SHARED_ENCODER:
                ...
                retrieval_latent_codes = retrieval_latent_codes.unsqueeze(
                    0
                ).repeat(len(SOURCE_MODEL_INFO), 1, 1)
                retrieval_latent_codes = retrieval_latent_codes.view(
                    -1, retrieval_latent_codes.shape[-1]
                )
            ...
            retrieval_latent_codes = retrieval_latent_codes.view(
                len(SOURCE_MODEL_INFO), -1, TARGET_LATENT_DIM
            )
            ...
            if USE_SRC_ENCODER_RETRIEVAL:
                with torch.no_grad():
                    ...
                    for j in range(num_sets):
                        if j == num_sets - 1:
                            curr_src_latent_codes = (
                                get_source_latent_codes_encoder(
                                    source_labels[j * interval :],
                                    SOURCE_MODEL_INFO,
                                    retrieval_encoder,
                                    device=device,
                                )
                    ...
                    src_latent_codes = torch.cat(src_latent_codes).view(
                        len(SOURCE_MODEL_INFO), -1, SOURCE_LATENT_DIM
                    )

            else:
                src_latent_codes = get_source_latent_codes_fixed(
                    source_labels, RETRIEVAL_SOURCE_LATENT_CODES, device=device
                )
                src_latent_codes = src_latent_codes.view(
                    len(SOURCE_MODEL_INFO), -1, SOURCE_LATENT_DIM
                )

            if DIST_FUNC == "mahalanobis":
                src_variances = get_source_latent_codes_fixed(
                    source_labels, SOURCE_VARIANCES, device
                )
                src_variances = src_variances.view(
                    len(SOURCE_MODEL_INFO), -1, SOURCE_LATENT_DIM
                )
            ...
            if DIST_FUNC == "mahalanobis":
                ...
                if ACTIVATION_FN.lower() == "none":
                    distances = compute_mahalanobis(
                        retrieval_latent_codes, src_latent_codes, src_variances
                    )
                ...
                if ACTIVATION_FN.lower() == "none":
                    distances = order_embedding_distance(
                        retrieval_latent_codes, src_latent_codes, device=device
                    )
                elif ACTIVATION_FN == "sigmoid":
                    distances = order_embedding_distance(
                        retrieval_latent_codes,
        ...
        # print(retrieved_idx.shape)
        # exit()
        retrieved_idx_repeated = (
            retrieved_idx.unsqueeze(0)
            .unsqueeze(-1)
            .repeat(1, 1, params.shape[-1])
        )
        ...
        (
            ...
            src_faces,
            src_face_labels,
        ) = get_source_info_visualization(
            retrieved_idx, SOURCE_MODEL_INFO, mesh=True
        )
        ...
        for j in range(cd_retrieved.shape[0]):
            ...
            # For graph 2
            cd_threshold_x_axis_tiled = np.tile(
                np.expand_dims(cd_threshold_x_axis, -1),
                (1, cd_loss_sorted[j].shape[0]),
            )

            count_greater = np.greater(
                cd_threshold_x_axis_tiled, cd_loss_sorted[j]
            )
            ...
            if j < 2 and MESH_VISU:
                ...
                output_vertices = get_shape_numpy(
                    curr_mat,
                    curr_param,
                    curr_default_param,
                    ALPHA,
                    connectivity_mat=curr_conn_mat,
                )
    ...
    log_string("")
    log_string(
        "Number of unique selected sources: "
        + str(len(used_sources))
        + "/"
        + str(len(SOURCE_MODEL_INFO))
    )
    log_string("")
    ...
    per_rank_total_cd_error_retrieved = (
        per_rank_total_cd_error_retrieved / num_evaluated
    )
    ...
    plt.scatter(
        ranking_x_axis,
        per_rank_total_cd_error,
        label="curr_deformation_function_ranking",
        c="b",
        s=1,
    )
    plt.scatter(
    ...
    dict_value = {
        "curr_deformation_function_ranking": per_rank_total_cd_error,
    ...
    for i in range(5):
        log_string(
            ...
            + str(i + 1)
            + ": "
            + str(per_rank_total_cd_error_retrieved[i])
        )
    ...
    for i in range(5):
        log_string(
            "\tRank " + str(i + 1) + ": " + str(per_rank_total_cd_error[i])
        )
    ...
    dict_value = {
        "thresholds": cd_threshold_x_axis,
