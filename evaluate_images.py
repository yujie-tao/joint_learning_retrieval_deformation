...
parser.add_argument(
    "--logdir",
    default="log_table_keypoint_images_AJ3_srcenc_symm/",
    help="path to the log directory",
    type=str,
)
parser.add_argument(
    "--dump_dir",
    default="fixed2_dump_table_keypoint_images_AJ3_srcenc_symm/",
    type=str,
)
    ...
    for source_model in sources:
        ...
        if USE_CONNECTIVITY:
            ...
            (
                ...
                faces,
                face_labels,
            ) = get_model(os.path.join(src_data_fol, src_filename), mesh=True)
    ...
    for i, batch in enumerate(loader):
                    ...
                    for j in range(num_sets):
                        if j == num_sets - 1:
                            curr_src_latent_codes = (
                                get_source_latent_codes_encoder_images(
        ...
        for j in range(cd_retrieved.shape[0]):
            ...
            if j < 2 and MESH_VISU:
                ...
                output_visualization_mesh_images(
                    output_vertices,
                    src_vertices[j],
                    src_faces[j],
                    target_shapes[j],
