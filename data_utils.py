...
IMAGE_BASE_DIR = (
    "orion/downloads/partnet_dataset/partnet_rgb_masks_storagefurniture/"
)
...
def save_dataset(fname, pcs, labels, semantics, model_ids):
    ...
    fout.create_dataset(
        "label", data=cloud_label, compression="gzip", dtype="int"
    )
    fout.create_dataset(
        "semantic", data=cloud_semantics, compression="gzip", dtype="int"
    )
    fout.create_dataset(
        "model_id", data=cloud_id, compression="gzip", dtype="float32"
    )
    fout.close()
...
def save_dataset_classification(
    fname, pcs, labels, semantics, model_ids, class_labels
):
    cloud = np.stack([pc for pc in pcs])
    cloud_label = np.stack([label for label in labels])
    cloud_semantics = np.stack([semantic for semantic in semantics])
    ...
    fout.create_dataset(
        "class_labels", data=c_class_labels, compression="gzip", dtype="int"
    )
    fout.close()
...
def output_visualization(
    ...
    # Source
    # Save point cloud.
    out_point_cloud_file = os.path.join(
        temp_fol, str(src_id) + "_" + str(target_id) + "_points.xyz"
    )
    np.savetxt(out_point_cloud_file, src_points, delimiter=" ", fmt="%f")
    print("Saved '{}'.".format(out_point_cloud_file))

    # Save point ids.
    out_point_ids_file = os.path.join(
        temp_fol, str(src_id) + "_" + str(target_id) + "_point_ids.txt"
    )
    ...
    render_point_cloud(
        out_point_cloud_file, out_point_ids_file, src_points_snapshot_file
    )
    ...
    # Render point_cloud.
    target_points_snapshot_file = os.path.join(
        temp_fol, str(target_id) + "_points"
    )
    render_point_cloud(
        out_point_cloud_file, out_point_ids_file, target_points_snapshot_file
    )
    ...
    # Render point_cloud.
    out_points_snapshot_file = os.path.join(
        temp_fol, str(src_id) + "_" + str(target_id) + "_points"
    )
    render_point_cloud(
        out_point_cloud_file, out_point_ids_file, out_points_snapshot_file
    )
...
def output_visualization_mesh(
    ...
    # Save vertex ids.
    out_vertex_ids_file = os.path.join(
        temp_fol, str(src_id) + "_vertex_ids.txt"
    )
    ...
    out_mesh_file = os.path.join(
        temp_fol, str(src_id) + "_" + str(target_id) + "_mesh.obj"
    )
    ...
    # Save vertex ids.
    out_vertex_ids_file = os.path.join(
        temp_fol, str(src_id) + "_" + str(target_id) + "_vertex_ids.txt"
    )
    np.savetxt(out_vertex_ids_file, output_vertices, fmt="%d")
    print("Saved '{}'.".format(out_vertex_ids_file))

    # Render mesh.
    output_mesh_snapshot_file = os.path.join(
        temp_fol, str(src_id) + "_" + str(target_id) + "_mesh"
    )
...
def output_visualization_mesh_images(
    ...
    img_filename = os.path.join(
        IMAGE_BASE_DIR,
        str(int(target_id)),
        "view-" + str(int(random_view[0])).zfill(2),
        "shape-rgb.png",
    )
    ...
    padded_img.paste(
        img_resized, ((width - new_size[0]) // 2, (height - new_size[1]) // 2)
    )
...
def output_visualization_mesh_naturalimages(
    output_vertices,
    src_vertices,
    src_faces,
    src_face_labels,
    ...
    # Save mesh.
    out_mesh_file = os.path.join(
        temp_fol, str(src_id) + "_" + str(counter) + "_mesh.obj"
    )
    mesh = trimesh.Trimesh(vertices=output_vertices, faces=src_faces)
    mesh.export(out_mesh_file, os.path.splitext(out_mesh_file)[1][1:])
    print("Saved '{}'.".format(out_mesh_file))
    ...
    # Save vertex ids.
    out_vertex_ids_file = os.path.join(
        temp_fol, str(src_id) + "_" + str(counter) + "_vertex_ids.txt"
    )
    np.savetxt(out_vertex_ids_file, output_vertices, fmt="%d")
    print("Saved '{}'.".format(out_vertex_ids_file))
    ...
    # Render mesh.
    output_mesh_snapshot_file = os.path.join(
        temp_fol, str(src_id) + "_" + str(counter) + "_mesh"
    )
    render_mesh(out_mesh_file, out_face_ids_file, output_mesh_snapshot_file)
    ...
    output_image_filename = os.path.join(
        output_fol, str(src_id) + "_" + str(counter) + "_deform.png"
    )
...
def output_visualization_mesh_images_mds(
    src_vertices,
    src_faces,
    target_points,
    src_face_labels,
...
def get_shape_numpy(
    A, param, src_default_param=None, weight=1.0, connectivity_mat=None
):
...
def get_source_info(
    source_labels, source_model_info, max_num_params, use_connectivity=False
):
    """
    source_labels: contain the labels on which sources are assigned to each target
    source_model_info: dictionary containing the info of the source models such as matrix A and default params
    ...
    for source_label in source_labels:
        ...
        padded_mat = np.zeros((points_mat.shape[0], max_num_params))
        padded_mat[
            0 : points_mat.shape[0], 0 : points_mat.shape[1]
        ] = points_mat
        # padded_mat = np.expand_dims(padded_mat, axis=0)
        ...
        if use_connectivity:
            constraint_proj_mat = source_model_info[source_label][
                "constraint_proj_mat"
            ]
            constraint_padded_mat = np.zeros((max_num_params, max_num_params))
            constraint_padded_mat[
                0 : constraint_proj_mat.shape[0],
                0 : constraint_proj_mat.shape[1],
            ] = constraint_proj_mat

            constraint_padded_mat = torch.from_numpy(
                constraint_padded_mat
            ).float()
...
def get_source_latent_codes_fixed(source_labels, latent_codes, device):
    ...
    src_latent_codes = torch.gather(
        latent_codes,
        0,
        source_labels.unsqueeze(-1).repeat(1, latent_codes.shape[-1]),
    )

    return src_latent_codes


def get_source_latent_codes_encoder(
    source_labels, source_model_info, encoder, device
):
...
def get_source_latent_codes_encoder_images(
    source_labels, source_model_info, encoder, device, obj_cat, random=False
):
    # print("Using encoder to get source latent codes.")
    img_size = 224
    source_images = []
    ...
    for source_label in source_labels:
        ...
        if random:
            ...
            IMAGE_BASE_DIR = (
                "orion/downloads/partnet_dataset/partnet_rgb_masks_"
                + obj_cat
                + "/"
            )
            img_filename = os.path.join(
...
def get_random_labels(source_labels, num_source_models):
    random_source_labels = np.random.randint(
        0, num_source_models, size=source_labels.shape
    )
